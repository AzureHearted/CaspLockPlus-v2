;
; cJson.ahk 2.1.0
; 版权所有 (c) 2023 Philip Taylor（又名 GeekDude, G33kDude）
; 项目地址：https://github.com/G33kDude/cJson.ahk
;
; 0BSD 许可证
;
; 允许任何人以任何目的使用、复制、修改和/或分发本软件，
; 无论是否收费，均无需获得授权。
;
; 本软件“按原样”提供，作者不对本软件作任何担保，
; 包括但不限于对适销性或特定用途适用性的任何默示担保。
; 在任何情况下，作者均不对因使用本软件而导致的任何特殊、
; 直接、间接或附带损害承担责任，包括但不限于使用损失、
; 数据丢失或利润损失，无论是合同、过失或其他侵权行为引起的，
; 亦不论是否因使用或与本软件的使用或性能有关。
;
...
class JSON
{
	...
	/**
	 * 当为 true 时，JSON 中的布尔值将被解码为数字：
	 * true → 1，false → 0。
	 * 
	 * 当为 false 时，JSON 中的布尔值将被解码为引用：
	 * true → {@link JSON.True}，
	 * false → {@link JSON.False}。
	 * 
	 * 默认情况下，该属性为 true。
	 */
	static BoolsAsInts {
	...
	/**
	 * 当为 true 时，JSON 中的 null 值将被解码为空字符串 ''。
	 * 
	 * 当为 false 时，JSON 中的 null 值将被解码为引用：
	 * {@link JSON.Null}。
	 * 
	 * 默认情况下，该属性为 true。
	 */
	...
	/**
	 * 当为 true 时，JSON 中的 Unicode 字符将使用反斜杠转义序列进行编码，
	 * 例如字符 '💩' 将被编码为 "\ud83d\udca9"。
	 * 这样做是为了提高与外部系统的兼容性。
	 * 
	 * 当为 false 时，Unicode 字符将保留为原始字符。
	 * 
	 * 默认情况下，该属性为 true。
	 */
	...
	/**
	 * 用于将非字符串值转换为字符串的实用函数（供 MCode 使用）。
	 */
	...
	static _LoadLib32Bit() {
		...
		if (32 != A_PtrSize * 8)
			...
			throw Error("$Name 不支持 " (A_PtrSize * 8) " 位 AHK，请使用 32 位 AHK 运行")
		...
		if !DllCall("Crypt32\CryptStringToBinary", "Str", codeB64, "UInt", 0, "UInt", 1, "Ptr", buf := Buffer(5816), "UInt*", buf.Size, "Ptr", 0, "Ptr", 0, "UInt")
			...
			throw Error("无法将 MCL 的 base64 数据转换为二进制")
		if (r := DllCall("ntdll\RtlDecompressBuffer", "UShort", 0x102, "Ptr", code, "UInt", 9904, "Ptr", buf, "UInt", buf.Size, "UInt*", &DecompressedSize := 0, "UInt"))
			...
			throw Error("调用 RtlDecompressBuffer 时出错", , Format("0x{:08x}", r))
		for import, offset in Map(['OleAut32', 'SysFreeString'], 8148) {
			if !(hDll := DllCall("GetModuleHandle", "Str", import[1], "Ptr"))
				...
				throw Error("无法加载 DLL " import[1] "： " OsError().Message)
			if !(pFunction := DllCall("GetProcAddress", "Ptr", hDll, "AStr", import[2], "Ptr"))
				...
				throw Error("无法在 " import[1] ".dll 中找到函数 " import[2] "： " OsError().Message)
	...
	static _LoadLib64Bit() {
		...
		if (64 != A_PtrSize * 8)
			...
			throw Error("$Name 不支持 " (A_PtrSize * 8) " 位的 AHK，请使用 64 位 AHK 运行")
		; MCL standalone loader https://github.com/G33kDude/MCLib.ahk
		; Copyright (c) 2023 G33kDude, CloakerSmoker (CC-BY-4.0)
		; https://creativecommons.org/licenses/by/4.0/
		if IsSet(lib)
	...
	/**
	 * 将对象转换为 JSON 字符串并写入指定文件。
	 * 
	 * @param obj 要写入的对象。
	 * @param {String} path 输出文件路径。
	 * @param {Integer} pretty 可选：是否美化输出 JSON 字符串（默认值为 0）。
	 * @param {String} encoding 可选：文件编码格式（如 "UTF-8"、"UTF-16" 等）。
	 * @returns {Integer} 实际写入的字符数。
	 */
	 ...
	 * 
	 * @param obj 要转换的对象
	 * @param pretty 是否美化输出 JSON 字符串（默认值：0）
	 * 
	 * @return 转换后的 JSON 字符串
	 */
	static Dump(obj, pretty := 0)
	{
		variant_buf := Buffer(24, 0) ; 创建一个足够大的缓冲区，用于存放 VARIANT 结构。
		var := ComValue(0x400C, variant_buf.ptr)  ; 创建一个指向 VARIANT 的引用。
		var[] := obj
		...
		; 如果一个 VARIANT 包含字符串或对象，
		; 必须显式释放它，可以通过调用 VariantClear，
		; 或者为其赋值为一个纯数字来实现：
		var[] := 0
	 ...
	 * 从文件中读取JSON字符串并解析为对象
	 * @param {String} json JSON字符串
	 * @param options 同 {@link FileRead} 的 options
	 */
	...
	/**
	 * 作为 JSON 中 “null” 的替代对象，
	 * 因为 AHK 没有原生的 null 类型。
	 * 
